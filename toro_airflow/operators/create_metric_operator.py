...
class UpsertMetricOperator(BaseOperator):
    @apply_defaults
    def __init__(self,
                 connection_id: str,
                 warehouse_id: int,
                 configuration: list(dict(schema_name=None, table_name=None, column_name=None,
                                          metric_name=None, threshold_configuration=None,
                                          extras=...)),
                 *args,
                 **kwargs):
    ...
    def execute(self, context):
        for c in self.configuration:
            ...
            thresholds = self._get_thresholds(c)
    ...
    def _get_existing_metric(self, table, column_name, metric_name):
        ...
        metrics = result.json()
        for m in metrics:
            if self._is_same_metric(m, metric_name) and self._is_same_column_metric(m, column_name):
                return m
        return None
    ...
    def _is_same_metric(self, metric, metric_name):
        keys = ["metricType", "predefinedMetric", "metricName"]
        result = reduce(lambda val, key: val.get(key) if val else None, keys, metric)
        if result is None:
            return False
        if metric_name.upper().startswith("HOURS_SINCE_MAX"):
            return result.startswith("HOURS_SINCE_MAX")
        else:
            return result.lower() == metric_name.lower()
